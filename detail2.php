...
class View extends \Car\Tools\View
{
    ...
    public function vehicle($index)
    {
        ...
        if (is_file(ROOT_PATH.DATA_FOLDER.'car/'.$index->id.self::$cfg->stored_img_type)) {
            $content .= '<figure class="center"><img src="'.WEB_URL.DATA_FOLDER.'car/'.$index->id.self::$cfg->stored_img_type.'"></figure>';
        }
        ...
        if (isset($index->mileage_start)) {
            $content .= '<tr><th>{LNG_Mileage Start}</th><td>'.$index->mileage_start.' km/h</td></tr>';
        }
    ...
    public function booking($index)
    {
        ...
        if ($index->phone != '') {
            $content .= ' ('.self::showPhone($index->phone, !$login).')';
        }
        $content .= '</td></tr>';
        ...
        if ($index->chauffeur > 0) {
            ...
            $chauffeur = [-1 => '{LNG_Self drive}', 0 => '{LNG_Not specified} ({LNG_anyone})'];
            $content .= $chauffeur[$index->chauffeur];
        }
        $content .= '</td></tr>';
        $content .= '<tr><th class=top>{LNG_Date}</th><td>';
        $content .= self::dateRange(['begin' => $index->begin, 'end' => $index->end]);
        $content .= '</td></tr>';
        ...
        if ($index->status == 1 && empty($index->is_calendar)) {
            // ซ่อนปุ่ม "คืนรถ" ถ้าอยู่ในหน้ารายละเอียดการจอง
            if (!isset($_GET['module']) || $_GET['module'] !== 'car-booking-detail') {
                $content .= '<tr><td colspan="2" class="center">';
                $content .= '<a href="updatemile.php?id=' . $index->id . '" class="button green">คืนรถ</a>';
                $content .= '</td></tr>';
            }
        }
