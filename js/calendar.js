...
Calendar.prototype = {
  ...
  _resize: function() {
    var cw = this.calendar.getClientWidth(),
      w = cw / 7;
    document.css('#' + this.id + ' td{height:' + w + 'px}', this.id);
  },
  _drawMonth: function() {
  ...
  _addLabel: function(d, prop, c) {
    ...
    if ($E(id)) {
      ...
      if (prop.color) {
        const c = new Color(prop.color);
        a.style.backgroundColor = c.toString();
        a.style.color = c.invert(true).toString();
      }
      a.className = c;
      a.datas = prop;
      div.appendChild(a);
      ...
      if (span.length == 0) {
        ...
        span.onclick = function() {
          ...
          forEach(div.getElementsByTagName('a'), function() {
            ...
            if (this.datas.color) {
              const c = new Color(this.datas.color);
              a.style.backgroundColor = c.toString();
              a.style.color = c.invert(true).toString();
            }
            if (this.datas.url) {
